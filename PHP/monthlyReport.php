...
$stmt = "SELECT COUNT(*) AS freq, `order`.supplierID FROM `order` WHERE `order`.`orderDate` BETWEEN (CURDATE() - INTERVAL 1 MONTH) AND CURDATE() GROUP BY supplierID";
$result = $con->query($stmt);
...
$stmt = $con->prepare("SELECT COUNT(*) AS orderfreq FROM `order` WHERE `order`.`orderDate` BETWEEN (CURDATE() - INTERVAL 1 MONTH) AND CURDATE()");
$stmt->execute();
$stmt->bind_result($orderamount);
$stmt->fetch();
$stmt->close();

$stmt = $con->prepare("SELECT COUNT(*) FROM orderedproduct WHERE orderedproduct.deliveredOn BETWEEN (CURDATE() - INTERVAL 1 MONTH) AND CURDATE()");
$stmt->execute();
$stmt->bind_result($delivamount);
$stmt->fetch();
$stmt->close();

$stmt = $con->prepare("SELECT CONCAT('£', CAST(AVG(orderTotalCost) AS DECIMAL(10,2))) FROM (SELECT `order`.`orderTotalCost` FROM `order` WHERE `order`.`orderDate` BETWEEN (CURDATE() - INTERVAL 1 MONTH) AND CURDATE()) AS q1");
$stmt->execute();
...
$stmt=$con->prepare("SELECT COUNT(*) FROM request WHERE request.requestState = 'Pending' AND request.`date` BETWEEN (CURDATE() - INTERVAL 1 MONTH) AND CURDATE()");
$stmt->execute();
$stmt->bind_result($currentPending);
$stmt->fetch();
$stmt->close();

$stmt = $con->prepare("SELECT COUNT(*) FROM request WHERE request.requestState = 'Authorised' AND request.`date` BETWEEN (CURDATE() - INTERVAL 1 MONTH) AND CURDATE()");
$stmt->execute();
$stmt->bind_result($currentAuth);
$stmt->fetch();
$stmt->close();

$stmt = $con->prepare("SELECT COUNT(*) FROM request WHERE request.requestState = 'Confirmed' AND request.`date` BETWEEN (CURDATE() - INTERVAL 1 MONTH) AND CURDATE()");
$stmt->execute();
$stmt->bind_result($currentConf);
$stmt->fetch();
$stmt->close();

$stmt = $con->prepare("SELECT COUNT(*) FROM request WHERE request.requestState = 'Issue' AND request.`date` BETWEEN (CURDATE() - INTERVAL 1 MONTH) AND CURDATE()");
$stmt->execute();
$stmt->bind_result($currentIss);
$stmt->fetch();
$stmt->close();

$stmt = $con->prepare("SELECT COUNT(*) FROM `order` WHERE orderState = 'Shipped' AND shippedOn BETWEEN (CURDATE() - INTERVAL 1 MONTH) AND CURDATE()");
$stmt->execute();
$stmt->bind_result($shippedandwaiting);
$stmt->fetch();
$stmt->close();

$stmt = $con->prepare("SELECT COUNT(*) FROM `order` WHERE orderState = 'Pending' AND orderDate BETWEEN (CURDATE() - INTERVAL 1 MONTH) AND CURDATE()");
$stmt->execute();
...
$graph->Add($bplot);
$graph->Stroke("./monthstaffreq.png");
...
$suppliergraph->Stroke("./monthsupplieramnt.png");
...
$pdf->Cell(0, 10, 'Gadgets4U Monthly Report - Generated by ' . $_SESSION['name'], 0, 1, "C");
$pdf->Cell(0, 10, $lastMonth . " - " . date("d/m/Y"), 0, 1, "C");
...
$pdf->Cell(0, 10, "Current Pending Requests Placed In Last Month: " . $currentPending, 0, 1);
$pdf->Cell(0, 10, "Current Authorised Requests Placed In Last Month: ". $currentAuth, 0, 1);
$pdf->Cell(0, 10, "Current Confirmed Requests Placed In Last Month: " . $currentConf, 0, 1);
$pdf->Cell(0, 10, "Current Requests Place in Last Month With Issues Flagged: " . $currentIss, 0, 1);
$pdf->Ln();
$pdf->Image("monthstaffreq.png", 40);
$pdf->Ln();
$pdf->SetFont("Arial", "BU", 12);
$pdf->Cell(0, 10, "Orders", 0, 1);
$pdf->SetFont("Arial", "", 12);
$pdf->Cell(0, 10, "Amount of Orders Placed in Last Month: " . $orderamount, 0, 1);
$pdf->Cell(0, 10, "Amount of Deliveries in Last Month: " . $delivamount, 0, 1);
$pdf->Cell(0, 10, "Average Order Cost: " . $avgordcost, 0, 1);
$pdf->Cell(0, 10, "Outstanding Orders Shipped This Month: " . $shippedandwaiting, 0, 1);
$pdf->Cell(0,10, "Orders Placed This Month and Not Shipped: " . $pending);
$pdf->Ln();
$pdf->Image("monthsupplieramnt.png", 40);
